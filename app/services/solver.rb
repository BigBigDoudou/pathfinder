class Solver
  ...
  def serialized_path
    serial = @serial
    generate_path[1..-2].each { |path| serial[path] = 'X' }
    serial
  end

  # ROOT METHOD TO FIND THE SHORTEST PATH AND TRACK TIME
  ...
  # RETURN THE SHORTEST PATH FOUND FROM START TO EXIT
  def generate_path
    node = @finish
    steps = []
    loop do
